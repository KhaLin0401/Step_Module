...
//     // Cập nhật jerk -> gia tốc
//     if (dv > 0) {
//         motion_state->j = motion_state->Jmax;   // tăng tốc
//     } else if (dv < 0) {
//         motion_state->j = -motion_state->Jmax;  // giảm tốc  
//     } else {
//         motion_state->j = 0;      // giữ nguyên
//     }
...
//     // Giới hạn gia tốc
//     if (motion_state->a > motion_state->Amax) motion_state->a = motion_state->Amax;
//     if (motion_state->a < -motion_state->Amax) motion_state->a = -motion_state->Amax;
...
//     // Giới hạn trong [0, Vmax]
//     if (motion_state->v_actual > motion_state->v_target) motion_state->v_actual = motion_state->v_target;
//     if (motion_state->v_actual < 0) motion_state->v_actual = 0;
...
//     // Tính ARR cho timer STEP
//     if (motion_state->v_actual > 0) {
//         uint32_t arr = (uint32_t)(SystemCoreClock / motion_state->v_actual);
//         if (arr > 0) TIM3->ARR = arr;
//     }
